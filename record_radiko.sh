...
if [ $# -eq 3 ]; then
  ...
  echo "usage : $0 channel_name duration(minuites) output_filename"
  exit 1
fi
...
rtmpdump -v \
         ...
         --live \
         --stop ${DURATION} \
         --flv "/tmp/radiko_${channel}_${date}.flv"

#avconv -i "/tmp/radiko_${channel}_${date}.flv" "${outfile}" && rm -f "/tmp/radiko_${channel}_${date}.flv"
avconv -i "/tmp/radiko_${channel}_${date}.flv" "${outfile}" # && rm -f "/tmp/radiko_${channel}_${date}.flv"
